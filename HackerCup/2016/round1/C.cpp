...
public:
    ...
    double solveSegment2(int l, int r, int limit) {
      assert(l < r && r <= period);
      int lp = limit % period;
      double ans = ((limit + period - r) / period) * 1.0 * (r - l) / 2.0 * (r - l);
      if (l <= lp && lp < r)
        ans += (lp - l) / 2.0 * (lp - l);
      return ans;
    }

    double solveSegment(int l, int r) {
      double res = solveSegment2(l, r, b) - solveSegment2(l, r, a);
      return res;
    }
    ...
    void solve() {
      int sum = 0;
      double ans = 0;
      period = (int) min(b + 1LL, accumulate(c.begin(), c.end(), 0LL));
      for (int i = 0; i < n; i++) {
        if (sum > b) break;
        if (c[i] != 0) {
          ans += solveSegment(sum, min(b+1,sum + c[i])) * 1.0 / (b - a);
          sum += c[i];
        }
      }
      printf("%.10f\n", ans);
