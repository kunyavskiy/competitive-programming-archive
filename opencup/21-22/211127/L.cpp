...
void get_segments(ll l, ll r, int k, ll L, ll R, vector<pair<ll, int>>& s) {
  if (r <= L || R <= l) return;
  if (L <= l && r <= R) {
    s.emplace_back(l, k);
    return;
  }
  get_segments(l, (l + r) / 2, k - 1, L, R, s);
  get_segments((l + r) / 2, r, k - 1, L, R, s);
}
...
void solve() {
  ...
  get_segments(0, 1LL << 40, 40, L, R + 1, s);
  sort(s.begin(), s.end(), [](const pair<ll, int>& a, const pair<ll, int>& b) {
  ...
  auto gauss = [&b](ll x) {
    ...
    if (x) {
      b.emplace_back(x, __builtin_ctzll(x));
      return true;
    }
    return false;
  };
  if (L <= 2 && 2 <= R) gauss(2);
  for (auto [l, blen] : s) {
    int curb_size = b.size();
    gauss(l + 1);
    for (int i = 1; i < blen; i++) {
      gauss(l + (1LL << i) + 1);
    }
    int newb_size = b.size();
    b.resize(curb_size);
    set<ll> checked;
    std::uniform_int_distribution<ll> dis(l, l + (1LL << blen) - 1);
    while ((int)b.size() < newb_size && (int)checked.size() < (1LL << blen)) {
      ll x = dis(rnd);
      if (checked.insert(x).second) {
        if (factorizer::IsPrime(x)) {
          gauss(x);
          //fprintf(stderr, "Found %lld: %zd/%d in basis now, checked %zd\n", x, b.size(), newb_size, checked.size());
        }
      }
    }
  }
