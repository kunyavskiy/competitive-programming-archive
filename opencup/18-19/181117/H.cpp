...
void solve(ll n) {
  ...
  vector<vector<pair<ll, int>>> dd(divs.size());
  for (int i = 0; i < (int)divs.size(); i++) {
    for (auto x : md) {
      if (divs[i] % x.first == 0) {
        dd[i].emplace_back(id(divs[i] / x.first), -x.second);
      }
    }
  }

  eprintf("%d %d\n", (int)md.size(), (int)divs.size());
