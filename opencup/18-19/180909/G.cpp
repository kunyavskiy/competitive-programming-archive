...
int calc2(int from, int to, int id) {
  ...
  for (auto t : next[id]) {
    ...
    if (id == 0) {
      eprintf(" -= %d * %d * %d\n", r1, r2, cnk(moves1 + moves2, moves1));
    }
    sub(ans, r);
  }
...
int main() {
  ...
  for (int i = 0; i < m; i++) {
    sumf[i] = vector<int>(byid.size());
    for (int j = 0; j < (int)byid.size(); j++) {
      for (int k : byid[j].first) {
        sumf[i][j] += points[i][k];
      }
    }
    sums[i] = vector<int>(byid.size());
    for (int j = 0; j < (int)byid.size(); j++) {
      for (int k : byid[j].second) {
        sums[i][j] += points[i][k];
      }
    }
  }
  ...
  for (int i = 0; i < m; i++) {
    for (int j = 0; j < m; j++) {
      dp2[i][j] = vector<int>(byid.size(), -1);
      dp3[i][j] = vector<int>(byid.size(), -1);
    }
  }
