...
void process( int i ) {
	...
	if (pos < r[i]) {
		int V = value[i];
		if (x[l[i]] != V && x[(r[i] + l[i]) / 2] != V) { //} && x[r[i] - 1] != V) {
			// int zeroes = get_sum(r[i]) - get_sum(l[i]);
			int zeroes = get_sum(l[i], r[i]);// - get_sum(l[i]);
...
void go( Event e ) {
	...
	forn(j, deg[e.i]) {//} (int j = head[e.i]; j; j = Next[j])  {
		int i = ind[e.i][j];//to[j];
		if ((covered[i] -= dt) == 0) 
			process(i);
	}
...
int main() {
	...
	forn(i, 2 * N)
		ind[i] = new int[deg[i]], deg[i] = 0;
	forn(i, k0 + k1)
		if (r[i] - l[i] <= MAGIC)
				for (int L = l[i] + N, R = r[i] + N; L < R; L >>= 1, R >>= 1) {
					if (L & 1) push_back(L++, i);
					if (R & 1) push_back(--R, i);
				}
