...
struct Matrix {
	...
	Matrix(int n) : v(n, vector<ll>(n, INFLL)){
		for (int i = 0; i< n; i++)
			v[i][i] = 0;
	}
...
public:
    ll findMin(int n, vector <int> a, vector <int> b, vector <int> w, int C) {
    	for (int& x : a) x--;
    	for (int& x : b) x--;
        Matrix g(n);
        for (int i = 0; i < (int)a.size(); i++)
        	g[a[i]][b[i]] = min(g[a[i]][b[i]], ll(w[i]));
        for (int k = 0; k < n; k++)	
        	for (int i = 0; i < n; i++)	
        		for (int j = 0; j < n; j++)
...
/*        for (int i = 0; i < n; i++)
        ...
        for (int i = 0; i < (int)a.size(); i++)
        	for (int j = 0; j < n; j++)
        		res[j][b[i]] = min(res[j][b[i]], g[j][a[i]] - w[i]);
...
#include <cmath>
using namespace std;
bool KawigiEdit_RunTest(int testNum, int p0, vector <int> p1, vector <int> p2, vector <int> p3, int p4, bool hasAnswer, ll p5) {
