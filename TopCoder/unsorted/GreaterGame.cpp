...
public:
    double calc(vector <int> a, vector <int> b) {
        ...
        for (int i = 0; i < n; i++){
        	...
        	if (b[i] == -1) continue;
        	all.erase(b[i]);
        	auto it = my.lower_bound(b[i]);
        	if (it == my.end()) it = my.begin();
        	if (*it > b[i])	ans += 1;
        	my.erase(*it);
        }
        vector<int> other(all.begin(), all.end());
        for (int x : my) {
        	int id = lower_bound(other.begin(), other.end(), x) - other.begin();
        	ans += id * 1.0 / other.size();
        }
        return ans;
    }
};
