...
class SpaceWarDiv1 {
public:
    ll minimalFatigue(vector <int> a, vector <int> _b, vector<ll> _cnt) {
        ll L = 0;
        ll R = 1LL<<60;
        sort(a.begin(),a.end());
        vector<pair<int,ll> > b(_b.size());
        ...
        while (R - L > 1){
        	...
        	for (int i = 0; i < (int)a.size(); i++){
        		while (ptr < (int)b.size() && b[ptr].first <= a[i])
        			sum += b[ptr].second, ptr++;
        		sum = max(0LL,sum - mid);
        	}
        	if (sum == 0)
        		R = mid;
...
#include <vector>
using namespace std;
bool KawigiEdit_RunTest(int testNum, vector <int> p0, vector <int> p1, vector<ll> p2, bool hasAnswer, ll p3) {
	cout << "Test " << testNum << ": [" << "{";
