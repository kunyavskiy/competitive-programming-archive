...
const ll LINF = 1000000000LL * 1000000000LL + 1000LL;
...
ld dcalc(ll times,ll days){
    return dcalc(days/times)*(ld)(times-days%times) + dcalc(days/times+1)*(ld)(days%times) + F*(ld)times;
}
...
bool check(int days){
    ...
    while (r-l > 100){
        ...
        ld f2 = dcalc(y,days);
        /*if (f1 == f2){
            cerr << f1 <<" "<<f2<<" "<<" "<<dcalc((x+y)/2,days)<<" "<<x<<" "<<y<<" "<<days << endl;;
            cerr << calc(x,days) <<" "<<fixed << dcalc(x,days) << endl;
            assert(false);
        }*/
        if (f1 < f2)
            r = y;
